...
int main()
{
    ...
    while (1) {
        ...
        if (0 == state.ID.nop)
        {
            ...
            else if (opcode =="0x02")
            {
                ...
                cout<<"Jump not taken"<<endl;
                newState.EX.nop = 0;
                newState.ID.nop = 1;
                    
                newState.IF.PC = bitset<32>(((state.IF.PC.to_ulong()+4)&0xf0000000)^((Imm.to_ulong()<<2)&0x0fffffff));
                newState.IF.nop = 0;
    ...
    cout<<"IPC:\t"<<1.0*inscnt/cycle<<endl;
    cout<<"MEMUsage:\t"<<1.0*memcnt/cycle<<endl;
